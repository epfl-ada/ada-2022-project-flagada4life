...
For example : it seems obvious that the main character of most movies is a male — that women charaters are younger in general than men — or that the roles they play (their actions) are influenced by their gender. In this study, we will use the CMU movies summary dataset, which gathers various information on the actors, characters, movies, and the pre-processed movies summaries.  
